...
public class UserMealsUtil {
    public static void main(String[] args) {
        List<UserMeal> meals = Arrays.asList(
                ...
                new UserMeal(LocalDateTime.of(2020, Month.JANUARY, 30, 20, 0), "Ужин", 300),
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        return meals.stream().filter(meal -> TimeUtil.isBetweenHalfOpen(meal.getDateTime().toLocalTime(), startTime, endTime))
                .map(meal -> new UserMealWithExcess(meal.getDateTime(), meal.getDescription(), meal.getCalories(),
                        isExcessCalPerDay(meals, meal.getDateTime(), caloriesPerDay))).toList();
    }

    private static boolean isExcessCalPerDay(List<UserMeal> meals, LocalDateTime day, int caloriesPerDay) {
        int sumCalPerDay = meals.stream().filter(meal -> meal.getDateTime().getDayOfMonth() == day.getDayOfMonth()).mapToInt(UserMeal::getCalories).sum();
